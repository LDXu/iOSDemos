...
- (void)setSelected:(BOOL)selected {
    ...
    if (selected) {
        //self.contentView.backgroundColor = [UIColor AppBlackWithTransparencyColor];
        _titleLabel.textColor = [UIColor appRedColor];
    } else {
        //self.contentView.backgroundColor = [UIColor clearColor];
        _titleLabel.textColor = [UIColor appFontBrownColor];
    }
}
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = [UIColor appFontGrayColor];
  }
  return _titleLabel;
}
...
- (void)setupSubviews {
    ...
    //  [self.view addSubview:_separator];    // 去掉_separator
    
    CGFloat offsetX = self.itemWidth;
    for (int i = 0; i < self.viewControllers.count - 1; ++i) {
        [self.segmentBar addSubview:({
            UIView *view =
            [[UIView alloc] initWithFrame:CGRectMake(offsetX, 7, SEPERATOR_LINE_WIDTH, SEGMENT_BAR_HEIGHT - 14)];
            view.backgroundColor = [UIColor appSeparatorGrayColor1];
            view;
        })];
        offsetX += self.itemWidth + SEPERATOR_LINE_WIDTH;
    }
}
...
- (UIScrollView *)slideView {
    if (!_slideView) {
        ...
        _slideView.scrollEnabled = false; // 不允许滑动
...
- (UICollectionView *)segmentBar {
    if (!_segmentBar) {
        ...
        _segmentBar.scrollEnabled = false;
...
- (UIView *)indicator {
    if (!_indicator) {
        ...
        _indicator.backgroundColor = self.indicatorColor ? : [UIColor appRedColor];
    }
    return _indicator;
}
...
- (CGFloat)itemWidth {
    //if (!_itemWidth) {
    _itemWidth = (self.view.frame.size.width - SEPERATOR_LINE_WIDTH * (self.viewControllers.count-1))
...
- (UICollectionViewFlowLayout *)segmentBarLayout {
    if (!_segmentBarLayout) {
        ...
        _segmentBarLayout.minimumLineSpacing = SEPERATOR_LINE_WIDTH;
                  ...
                  cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    segmentBarItem.selected = false;
    if (indexPath.row == _selectedIndex) {
        segmentBarItem.selected = true;
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row != 0 && _selectedIndex == 0) {
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:0 inSection:0];
        UICollectionViewCell *cell = [self.segmentBar cellForItemAtIndexPath:indexPath];
        cell.selected = false;
    }
